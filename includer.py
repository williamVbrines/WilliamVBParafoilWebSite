...
def clean_data(data, delimiter):
    start_index = data.find("<" + delimiter + ">")
    end_index = -1 
    
    if start_index == -1:
        raise ValueError("ERROR delimiter <" + delimiter + "> not found in file")
        
    end_index = data.find("</" + delimiter + ">", start_index) 
    
    if end_index == -1:
        raise ValueError("ERROR delimiter </" + delimiter + "> not found in file")
        
    end_index += 3 + len(delimiter)
...
def include(data_path, file_path, delimiter = ""):
    ...
    data_include = clean_data(data_include, delimiter)
